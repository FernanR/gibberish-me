 ...
 *   It uses the jaylist wrapper (http://github.com/benbscholz/jaylist) for hash
 *   table functionality. 
 * 
...
/**
 * @constructor
 * GibberishMe: Gibberish generator. nPrefix is an optional argument.
 * The default is 2. 
 */
...
GibberishMe.prototype = {
    ...
    generateGibberish: function (numGen) {
        ...
        gibberish = prefix.concat(" ".concat(suffix.concat(" ")));
        ...
        for (i = 0; i < numGen; i = i + 1) {
            ...
            if (prefix.charAt(prefix.length - 1) === ".") {
                prefix = this.getPrefix();
                suffix = this.getSuffix(prefix);
                gibberish = gibberish.concat(
                                prefix.concat(" ".concat(suffix.concat(" "))));
                
            // if no suffix exists for the given prefix, generate a fresh prefix
            } else if (this.getSuffix(prefix) === false) {
                prefix = this.getPrefix();
                suffix = this.getSuffix(prefix);
                gibberish = gibberish.substring(0, (gibberish.length - 1));
                gibberish = gibberish.concat(". ".concat(prefix.concat(" ".concat(suffix))));
                
            // otherwise, generate a suffix as usual
            } else {
                suffix = this.getSuffix(prefix);
                gibberish = gibberish.concat(suffix.concat(" "));
                prefix = this.removePrefix(prefix).concat(" ".concat(suffix));
            }
        }
        return gibberish;
    ...
    getSuffix: function (prefix) {
        var suflist;   
        if (this.gibberTable.get(prefix) === undefined)
            return false; 
        suflist = this.gibberTable.get(prefix);
        return suflist[Math.round(Math.random() * (suflist.length - 1))];
    },
     ...
     */ 
    getPrefix: function () {
        var preflist, randpref;
        preflist = this.gibberTable.keys();
        
        for (;;) {
            randpref = preflist[Math.round(Math.random() * (preflist.length - 1))];
            // if it begins with a capital letter return 
            if (randpref.charAt(0).toUpperCase() == randpref.charAt(0)) {
                return randpref;
            }
        }
    },
...
function tryObama() {
    window.document.gibberform.input.value = OBAMA_TEXT;
}
